# ============================================================
# Build VJ146A2 Reliability Panel (Yearly-Keep Variant)
# VJ Stage 5 — Reliability Panel with Annual Pre-filter
# ============================================================
#
# Purpose:  Pre-filters settlements to those electrified in a VJ-specific
#           yearly keep/stat file before running the rolling-window
#           reliability logic on VJ146A2 monthly panels.
# Inputs:   - Map Data/settlement_day_outputs_vj146a2/settlement_day_vj146a2_cov_yearlykeep_YYYY-MM.parquet
#           - Map Data/reliability_outputs_vj146a2/<VJ yearly keep/stat parquet>
# Outputs:  - Map Data/reliability_outputs_vj146a2/settlement_reliability_*.parquet
# Run:      Rscript Visualizer/reliability_panel_build_yearlykeep_vj146a2.R
# ============================================================
...
# VJ-specific yearly composite settlement keep-list or stats file.
# This script intentionally does not fall back to VNP production outputs.
...
YEARLY_KEEP_CANDIDATES <- c(
  Sys.getenv("VJ146A2_YEARLY_KEEP_FILE", ""),
  file.path(OUT_DIR, paste0("vj146a2_yearly_keep_", YEAR_LABEL, ".parquet")),
  file.path(OUT_DIR, paste0("yearly_settlement_keep_", YEAR_LABEL, ".parquet"))
)
YEARLY_STATS_CANDIDATES <- c(
  Sys.getenv("VJ146A2_YEARLY_STATS_FILE", ""),
  file.path(OUT_DIR, paste0("yearly_settlement_stats_", YEAR_LABEL, ".parquet")),
  file.path(OUT_DIR, paste0("vj146a2_yearly_settlement_stats_", YEAR_LABEL, ".parquet"))
)
...
YEARLY_KEEP_FILE <- pick_existing_path(YEARLY_KEEP_CANDIDATES)
YEARLY_STATS_FILE <- pick_existing_path(YEARLY_STATS_CANDIDATES)
YEARLY_SOURCE_KIND <- NA_character_
YEARLY_SOURCE_FILE <- NA_character_

if (!is.na(YEARLY_KEEP_FILE)) {
  YEARLY_SOURCE_KIND <- "vj_yearly_keep_list"
  YEARLY_SOURCE_FILE <- YEARLY_KEEP_FILE
} else if (!is.na(YEARLY_STATS_FILE)) {
  YEARLY_SOURCE_KIND <- "vj_yearly_stats"
  YEARLY_SOURCE_FILE <- YEARLY_STATS_FILE
} else {
  stop(
    "No VJ-specific yearly keep/stats file found. Run VJ pre-Stage 2 first.\n",
    "Expected one of:\n  - ",
    paste(c(YEARLY_KEEP_CANDIDATES, YEARLY_STATS_CANDIDATES)[nzchar(c(YEARLY_KEEP_CANDIDATES, YEARLY_STATS_CANDIDATES))], collapse = "\n  - "),
    "\nCommand: Rscript Builder/vj146a2_yearly_keep_build.R\n",
    "No fallback to VNP yearly stats is allowed."
  )
}

log_step("Using VJ yearly source:", YEARLY_SOURCE_KIND, YEARLY_SOURCE_FILE)
...
if (length(missing_files) > 0 && !ALLOW_PARTIAL) {
  stop(
    "Missing VJ yearly-keep monthly parquet file(s):\n  - ",
    paste(missing_files, collapse = "\n  - "),
    "\nRun VJ Stage 3b first: Rscript Builder/settlement_day_yearlykeep_filter_vj146a2.R",
    "\nFor an explicit partial diagnostic run, set VJ146A2_ALLOW_PARTIAL=1."
  )
}
if (length(existing_files) == 0) stop("No VJ yearly-keep monthly parquet files found in range.")
if (ALLOW_PARTIAL && length(missing_files) > 0) {
  log_step("VJ146A2_ALLOW_PARTIAL=1; reading", length(existing_files), "of", length(input_files), "configured monthly files.")
}
...
yearly_raw <- read_parquet(YEARLY_SOURCE_FILE)
if (!(YEARLY_KEEP_ID_COL %in% names(yearly_raw))) {
  stop("VJ yearly source is missing ID column '", YEARLY_KEEP_ID_COL, "': ", YEARLY_SOURCE_FILE)
}
if (!(YEARLY_KEEP_FLAG_COL %in% names(yearly_raw))) {
  if (identical(YEARLY_SOURCE_KIND, "vj_yearly_keep_list")) {
    yearly_raw[[YEARLY_KEEP_FLAG_COL]] <- 1L
  } else {
    stop("VJ yearly stats source is missing flag column '", YEARLY_KEEP_FLAG_COL, "': ", YEARLY_SOURCE_FILE)
  }
}

yearly_stats <- yearly_raw %>%
  mutate(
...
if (nrow(yearly_keep) == 0) {
  stop("No kept settlements found in VJ yearly source (", YEARLY_SOURCE_FILE, ").")
}
...
if (length(exclusion_dates) > 0) {
  n_rows_before_exclusion <- nrow(sett_day)
  n_dates_before_exclusion <- dplyr::n_distinct(sett_day$date)
  sett_day <- sett_day %>% filter(!date %in% exclusion_dates)
  log_step(
    "Applied excess-lit date exclusion:",
    length(exclusion_dates), "configured dates | rows:",
    n_rows_before_exclusion, "->", nrow(sett_day),
    "| observed dates:", n_dates_before_exclusion, "->", dplyr::n_distinct(sett_day$date)
  )
}
...
# by VJ pre-Stage 2 / vj146a2_yearly_keep_build.R (electrified_best == 1).
# The area_population column computed from this in compute_area_metrics() is
# consequently the population of VJ-yearly-keep settlements per area, the
...
message("VJ146A2 reliability pipeline complete (yearly keep-set mode).")
