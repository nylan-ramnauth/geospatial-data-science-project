...
# ============================================================
# 2023 VJ146A2 settlement-day MLR validation diagnostic
# ============================================================
#
# Diagnostic/trial path only. Does not touch production VNP files
# or the October exploratory Eskom MLR script outputs.
#
# Inputs:
...
#   VJ146A2_START_MONTH=2023-01 VJ146A2_END_MONTH=2023-03 Rscript Builder/vj146a2_2023_mlr_validation.R
#
# RStudio:
...
OUT_DIR <- file.path(
  ...
  Sys.getenv("VJ146A2_MLR_OUT_SUBDIR", DEFAULT_OUT_SUBDIR)
)

OUT_PANEL <- file.path(OUT_DIR, "vj146a2_2023_mlr_validation_daily_panel.csv")
OUT_COUNTS <- file.path(OUT_DIR, "vj146a2_2023_mlr_validation_event_counts_by_threshold.csv")
OUT_MODELS <- file.path(OUT_DIR, "vj146a2_2023_mlr_validation_models.csv")
OUT_ROBUSTNESS <- file.path(OUT_DIR, "vj146a2_2023_mlr_validation_robustness_windows.csv")
OUT_LARGE_EVENTS <- file.path(OUT_DIR, "vj146a2_2023_mlr_validation_large_events.csv")
OUT_RAVI_STYLE <- file.path(OUT_DIR, "vj146a2_2023_mlr_validation_ravi_style_table.csv")
OUT_REPORT_RMD <- file.path(OUT_DIR, "vj146a2_2023_mlr_validation_report.Rmd")
OUT_REPORT_PDF <- file.path(OUT_DIR, "vj146a2_2023_mlr_validation_report.pdf")
...
OLD_EXPLORATORY_PANEL <- ""

if (!file.exists(VJ_YEARLY_STATS_FILE)) {
  stop(
    "VJ yearly stats file not found: ", VJ_YEARLY_STATS_FILE,
    "\nRun VJ pre-Stage 2 first: Rscript Builder/vj146a2_yearly_keep_build.R",
    "\nNo fallback to VNP yearly stats is allowed."
  )
}
...
for (p in c(ESKOM_HOURLY, SETT_GPKG)) {
...
missing_yearly_cols <- setdiff(required_yearly_cols, names(yearly_keep))
...
sett_day <- sett_day %>%
  ...
  mutate(
    ...
    electrified_best = electrified_best_yearly
  ) %>%
  select(-population_yearly, -electrified_best_yearly)

if (nrow(sett_day) == 0) {
  stop("No VJ settlement-day rows remain after applying electrified_best == 1 from VJ pre-Stage 2.")
}
...
large_events <- sett_day %>%
  ...
  select(
    ...
    mostly_dark,
    p_lit_sett,
    analysis_median_p_lit,
    drop_from_analysis_median,
    coverage,
    analysis_obs_days,
    mean_rad_sett,
    median_rad_sett,
...
ravi_style_table <- bind_rows(
  ...
  mutate(
    ...
    sample_rule = case_when(
      model_type == "ravi_continuous_shed_share" ~ paste0(ANALYSIS_WINDOW_TEXT, " VJ product dates with matched Eskom exposure; VJ pre-Stage-2 yearly-keep settlements with population > 100"),
      model_type == "ravi_rank_top7_vs_bottom7" ~ "Seven highest vs seven lowest local-overpass shed-share days; VJ pre-Stage-2 yearly-keep settlements with population > 100",
      model_type == "ravi_tied_zero_bottom_quartile_sensitivity" ~ "All tied zero-MLR bottom-quartile days vs top-quartile days; VJ pre-Stage-2 yearly-keep settlements with population > 100",
      TRUE ~ NA_character_
    ),
...
# -----------------------------
caption_base <- paste(
  paste0("VJ146A2 ", format(month_start, "%Y-%m-%d"), " to ", format(analysis_end, "%Y-%m-%d"), ";"),
...
p_ts <- ggplot(ts_panel, aes(x = local_overpass_date, y = value, color = series, linetype = series)) +
  ...
  labs(
    ...
    subtitle = "Filtered events: coverage >= 0.8, analysis-period median p_lit >= 0.7, population >= 10,000; x-axis is local overpass date",
...
if (file.exists(OLD_EXPLORATORY_PANEL)) {
  ...
  old_style_daily <- sett_day %>%
    group_by(date) %>%
    summarise(
      old_large_dark_events_10k = sum(population >= HEADLINE_THRESHOLD & strict_dark, na.rm = TRUE),
      .groups = "drop"
    ) %>%
...
write_ravi_report_rmd <- function(path) {
  report_lines <- c(
    "---",
    paste0("title: \"VJ146A2 MLR Validation Report: ", ANALYSIS_WINDOW_TEXT, "\""),
    ...
    "ravi <- read.csv(\"vj146a2_2023_mlr_validation_ravi_style_table.csv\", check.names = FALSE)",
    ...
    "## Validation Question",
    "",
    paste0("The VJ146A2 validation panel for ", ANALYSIS_WINDOW_TEXT, " tests the relationship between local-overpass Eskom MLR and satellite-observed nighttime darkness."),
    "",
    "The validation question is whether days with higher national Eskom manual load reduction at the VIIRS local overpass window also show higher population-weighted darkness in the VJ pre-Stage-2 yearly-keep settlement panel. The headline outcome is the population-weighted share of people in dark settlements, restricted to yearly-keep settlements with population greater than 100.",
    "",
    "## Methodology",
    "",
    ...
    paste0("- Sample: ", ANALYSIS_WINDOW_TEXT, " VJ146A2 product dates with matched Eskom local-overpass exposure; VJ pre-Stage-2 `electrified_best == 1` settlements with population greater than 100."),
    "",
    "The continuous models use all matched VJ dates in the configured analysis window. The extreme-bin headline compares the seven highest vs seven lowest shed-share dates. The tied-zero bottom-quartile row is a robustness check because zero-MLR days are tied at the lower quartile.",
    "",
    "## Headline Table",
    "",
    "```{r table}",
    ...
    "## Interpretation",
    "",
    "The strongest report-facing specification is the population-weighted national share, because it asks how many people are in settlements that appear dark rather than how many settlement polygons cross a threshold.",
    "",
...
if (requireNamespace("rmarkdown", quietly = TRUE)) {
  render_ok <- tryCatch(
    ...
    error = function(e) {
      message("Skipping PDF report render after error: ", conditionMessage(e))
      FALSE
    }
  )
  if (render_ok && !file.exists(OUT_REPORT_PDF)) {
    message("PDF report render finished but file was not found: ", OUT_REPORT_PDF)
  }
} else {
  message("Package 'rmarkdown' is not installed; wrote Rmd but skipped PDF report render.")
}
...
if (file.exists(OUT_REPORT_PDF)) message("Wrote report PDF: ", OUT_REPORT_PDF)
